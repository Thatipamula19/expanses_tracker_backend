...
export class GoalsService {
  ...
  public async getGoalProgressOverview(
    user_id: string,
    dto: GetGoalsDashboardDto,
  ) {
    try {
      const where: FindOptionsWhere<Goal> = { user_id };
      ...
      const ongoingCount = goals.filter(
        (g) => g.status === GoalStatus.ONGOING,
      ).length;
      ...
      throw new InternalServerErrorException(
        'Failed to get goal progress overview',
      );
    }
  }
  ...
  public async addContribution(
    ...
    const goal = await this.goalRepository.findOne({
      where: { id: addContributionDto.goal_id, user_id },
    });
    ...
    goal.saved_amount =
      Number(goal.saved_amount) + Number(addContributionDto.amount);
  ...
  public async removeContribution(user_id: string, contribution_id: string) {
    ...
    if (contribution.goal.user_id !== user_id)
      throw new ForbiddenException(
        'Not authorized to delete this contribution',
      );

    const goal = contribution.goal;
    goal.saved_amount = Math.max(
      Number(goal.saved_amount) - Number(contribution.amount),
      0,
    );
  ...
  public async removeGoal(user_id: string, goal_id: string) {
    try {
      ...
      if (!goal) throw new NotFoundException('Goal not found');
      if (goal.user_id !== user_id)
        throw new ForbiddenException('Not authorized to delete this goal');
  ...
  private resolveGoalTimePeriod(period: GoalTimePeriod): {
    start: Date | null;
    end: Date | null;
  } {
    const now = new Date();
