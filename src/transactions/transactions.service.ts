...
import { DateRangePeriod, OverviewPeriod, SortOrder, TransactionType } from '@/common/enums';
...
export class TransactionsService {
    ...
    public async getOverview(user_id: string, dto: GetOverviewDto) {
        try {
            ...
            const income_vs_expense = Array.from(monthMap.entries()).map(([key, val]) => {
            ...
            const totalExpense = [...categoryMap.values()].reduce((s, c) => s + c.amount, 0);
            ...
            throw new InternalServerErrorException('Failed to get transactions overview');
        }

    }

    public async getCategoryWiseExpenses(user_id: string, dto: GetCategoryWiseExpensesDto) {
    try {
        const now         = new Date();
        ...
        return {
            ...
            summary: {
                ...
                over_budget_count:   categoryData.filter(c => c.is_over_budget).length,
        ...
        throw new InternalServerErrorException('Failed to get category wise expenses');
    }
}
